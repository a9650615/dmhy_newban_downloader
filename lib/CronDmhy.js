...
export default new class CronDmhy {
   ...
   */
	...
	async searchDmhyList(url, largeThanEpisode = 0, largeThanDate = null) {
		...
		$('.tablesorter>tbody>tr').each((index, element) => {
			...
			if (itemData.episode.length == 1 && itemData.episode[0] < largeThanEpisode) {
				return
			}
  ...
  async searchXML( searchData = {
    ...
    opt = Object.assign({}, options);
		...
		return await this.searchDmhyList(opt.url, searchData.largeThanEpisode, searchData.largeThanDate)
