...
export default new class UploadGD {
  ...
  async uploadFileToGD(files, nameInJpn) {
    ...
    for(const file of files) {
      ...
      if (folderPath == '') { // upload directly

      } else {
        const folderId = GoogleDriveDatabase.getFolderMapByPath(folderPath).folderId
        try {
          await this.uploadFileRecord(file.path, file.path.match(/[\/\\](.*)/)[1], null, folderId)
